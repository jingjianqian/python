...
class snake(Frame):
    def __init__(self, master=None):
        ...
        id = self.canvas.create_oval(self.body[0][0] * s, self.body[0][1] * s,
                                     ...
                                     outline="white")  # 绘制初始蛇头
        self.bodyid.insert(0, id)
        for i in range(1, len(self.body)):
            id = self.canvas.create_oval(self.body[i][0] * s, self.body[i][1] * s,
                                         (self.body[i][0] + 1) * s, (self.body[i][1] + 1) * s, fill="green",
                                         outline="green")  # 绘制初始蛇体
        ...
        if (self.bigfoodflg == 5):  # 如果吃到了第五个食物
            ...
            if (r != 0):  # 三分之二的可能产生奖励
                ...
                while ((x, y) in self.body or (x, y) == self.food):  # 确保食物和蛇体不冲突
                    x = random.randrange(0, self.gridcount)
                    y = random.randrange(0, self.gridcount)
                id = self.canvas.create_oval(x * s - 2, y * s - 2, (x + 1) * s + 2, (y + 1) * s + 2, fill="yellow",
                                             outline="blue")
                self.bigfood[0] = x
                self.bigfood[1] = y
                self.bigfoodid = id
    ...
    def keypress(self, event):
        if self.died == 1 or self.stop == 1:  # 如果玩家死亡或者处于暂停,返回
            return
        if (
                event.keysym == "Up" or event.keysym.upper() == "W") and self.di != 3 and self.lockey == 0:  # 如果玩家按上并且玩家正在前进的方向不是下
            self.di = 1  # 刷新方向为上
