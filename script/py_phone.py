...
class Element(object):
    ...
    def __uidump(self):
        ...
        获取当前Activity控件树
        """
        os.popen("adb shell uiautomator dump /storage/sdcard0/script/uidump.xml")
        os.popen("adb pull /storage/sdcard0/script/uidump.xml " + self.tempFile)
    ...
    def __element(self, attrib, name):
        ...
        """
        self.__uidump()
        #print(self.tempFile + "\\uidump.xml")
    ...
    def __elements(self, attrib, name):
        ...
        list = []
        self.__uidump()
        print(file=self.tempFile + "\\uidump.xml")
        tree = ET.ElementTree(file=self.tempFile + "\\uidump.xml")
        treeIter = tree.iter(tag="node")
        for elem in treeIter:
            if elem.attrib[attrib] == name:
...
def test():
    ...
    es = Element.findElementsByName(u"微信")
    if e1:
        evevt.touch(e1[0], e1[1])
        time.sleep(3)
    else:
        print("获取微信节点失败")
    # element_secend = Element()
    # e2 = element_secend.findElementByName(u"仙女")
